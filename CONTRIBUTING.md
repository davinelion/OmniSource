...
- Do not fabricate missing hashes, release dates, screenshots, or download
  mirrors. Screenshots must be declared from the app's **own upstream** (its
  repository, or the site the developer publishes from): pointing
  `catalog.json` at `assets/screenshots/…` inside OmniSource records no origin at
  all, and `scripts/validate.py` rejects it. Apps whose project publishes no art
  declare `screenshots: []` - the app page then says so plainly, and the icon is
  never rendered as a preview. `feeds/screenshots.json` is generated by the build
  (with a digest per mirrored file) and must describe files that exist: run the
  pipeline, never hand-edit it.
