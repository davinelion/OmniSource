...
| [`build-tweak.yml`](build-tweak.yml) | manual · reusable (`workflow_call`) | Release asset | Inject one `.deb` tweak — or an ordered bundle of them, one cyan pass each — into a decrypted base IPA; inputs are sanitized, https-only and policy-gated; every injected digest is returned as an output |
| [`tweak-factory.yml`](tweak-factory.yml) | schedule (weekly Mon 02:50 UTC) · manual | Releases (`tweak-build/…` tags), `data/tweak-builds-state.json` | Collect the newest official `.deb` per registry entry (GitHub release or the developer's own apt index), inject it or its whole bundle into the operator-supplied base app, publish a provenance release, then dispatch sync — see `docs/TWEAK-FACTORY.md` |
